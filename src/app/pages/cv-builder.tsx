...
export function CVBuilder() {
  ...
  return (
    <SidebarProvider defaultOpen className="h-screen bg-slate-100 text-slate-900">
      <Sidebar variant="inset" className="border-slate-200/80">
        <SidebarHeader className="gap-4 border-b border-slate-200/70 bg-white/90 px-3 py-4 backdrop-blur">
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50/80 p-3 shadow-sm">
            <div className="flex items-center justify-between text-[11px] font-medium uppercase tracking-[0.08em] text-slate-500">
              <span>Build progress</span>
              <span>{safeActiveSectionIndex + 1}/{builderSections.length}</span>
            </div>
            <Progress value={progressValue} className="mt-2 h-2 bg-slate-200" />
            <p className="mt-3 text-sm font-semibold text-slate-900">{showSettings ? 'Visual Settings' : activeSection.label}</p>
            <p className="mt-1 text-xs leading-relaxed text-slate-500">
              {showSettings
                ? 'Tweak template, layout, fonts and color choices while keeping the preview visible.'
                : activeSection.description}
            </p>
          </div>
        </SidebarHeader>

        <SidebarContent className="bg-white/70">
      ...
      <SidebarInset className="min-h-screen overflow-hidden bg-slate-100">
        {/* Main Content */}
          ...
          {showPreview && (
            <>
              ...
              <ResizablePanel defaultSize={54} minSize={30}>
                <div className="flex h-full flex-col overflow-hidden bg-slate-200/80">
